...
# 開発環境でのみ動作するように動作を切り替えます。
# 画像を静的ファイルとして扱う場合、urls.pyに「if settings.DEBUG:」を記述します。これは、Djangoが開発モード（DEBUG=True）で動作するときは、静的ファイルを直接表示できるようにするためです。
if settings.DEBUG:
